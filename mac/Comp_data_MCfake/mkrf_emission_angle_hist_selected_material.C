...
void mkrf_emission_angle_hist_selected_material(TString input = "", TString output = "", Int_t matflg = 0, Int_t pflg = 0,Double_t eve = 1, Double_t mceve = 1){
	...
	while(ifs>>ecc>>gid>>utime>>w>>vpl>>material>>ntrk>>ntrk_all>>vx>>vy>>vz>>dz>>cid>>pid>>stopflg>>
		...
		//if(pid!=13&&nseg/npl<0.60)continue;
	...
	std::cout<<"#muon = " << cnt / w0	  << " " << "after norm => " << cnt <<std::endl;
	std::cout<<"#pion = " << cnt_pi / w0<< " " << "after norm => " << cnt_pi * w0 <<std::endl;
